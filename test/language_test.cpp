...
TEST_F(VkPtrBasics, DoubleAssignShouldFail) {
  ...
  ASSERT_DEATH(secondI = &presetInst, "operator& before reset()");
...
TEST_F(VkPtrBasics, EmptyWhenCastToIntShouldFail) {
  ...
  ASSERT_EQ(bool(pInt), false);
  int n = 0;
  ASSERT_DEATH(n = pInt, "operator .* on an empty VkPtr!");
  ASSERT_EQ(n, 0);
}
...
TEST_F(VolcanoReflectionMapBasics, AddField) {
  int gotInt = 0;
  ASSERT_DEATH(if (reflect.get("fieldInt", gotInt)) exit(1),
               "get\\(fieldInt\\): field not found");
  ASSERT_EQ(reflect.addField("fieldInt", &fieldInt), 0);
  ASSERT_DEATH(if (reflect.addField("fieldInt", &fieldUInt32)) exit(1),
...
TEST_F(VolcanoReflectionMapBasics, GetSetInt) {
  ...
  ASSERT_DEATH(if (reflect.set("fieldInt", (uint32_t)TEST_VALUE_2)) exit(1),
               "setVkBool32\\(fieldInt\\): want type VkBool32, got int");
  ASSERT_DEATH(if (reflect.set("fieldUInt32", (int)TEST_VALUE_1)) exit(1),
...
class InstanceTests : public ::testing::Test {
 protected:
  ...
  void DoCtorError() {
    ...
    ASSERT_EQ(i.minSurfaceSupport.erase(language::PRESENT), true);
